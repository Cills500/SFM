...
Limg = imread('L_image.jpg');
Rimg = imread('R_image.jpg');

% Undistort images using precomputed camera intrinsics
load 'calibrationSession3.mat'
% Undistort images using precomputed camera intrinsics
Limg = undistortImage(Limg, stereoParams3.CameraParameters1);
Rimg = undistortImage(Rimg, stereoParams3.CameraParameters2);
...
figure
imshow(Limg, 'InitialMagnification', 50);
title('150 Strongest Corners from the Left Image');
hold on
...
figure
showMatchedFeatures(Limg, Rimg, matchedPoints1, matchedPoints2);
...
imagePoints1 = detectMinEigenFeatures(rgb2gray(Limg), 'ROI', roi, ...
    'MinQuality', 0.001);
